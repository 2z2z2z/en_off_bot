...
## Фаза 6 — Whitelist и админка

- Добавлен модуль `src/entities/user/service/whitelist-service.js`, инкапсулирующий работу с белым списком поверх `admin_config.json`: нормализует логины, гарантирует существование массива записей, синхронизирует Set-кэш, хранит метаданные (`role`, `addedBy`, `addedAt`) и предотвращает дубли. Сообщения об ошибках переведены на ASCII; тест `whitelist-service.test.js` обновлён под новые формулировки.
- `src/features/router.js` создаёт whitelist-сервис единожды, пробрасывает его в `createAdminMenu`/`createAdminCallbackHandler`, избавляет админские ответы от emoji и русскоязычных сообщений, а также проксирует `toggleModeration` в callback-слой.
- `src/features/admin/menu/index.js` переработан: обработчики whitelist используют сервис, отправляют явные ответы при ошибках/успехе и валидируют ручной ввод логина. Презентационные компоненты (`components/*.js`) переписаны на ASCII-контент (заголовки, кнопки, подсказки).
- `admin-handler.js` переключён на `toggleModeration`, очищен от дублирующих уведомлений и сообщает об ошибках по-английски, что устраняет конфликты кодировок и повторную отправку callback.
- Сообщения о доступе к админке и блокировке пользователей (`handleAdminAction`, `checkGameAccess`) приведены к английским формулировкам, чтобы унифицировать UX и пройти линтер по символам.
